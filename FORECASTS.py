...
class FORECASTS:
    ...
    def _normalize(self, st_df, normalized, logarithmized):
        """
        ...
        if not normalized:
            ...
            scanpy.pp.normalize_total(
                adata, exclude_highly_expressed = True
            )
            st_df = adata.to_df()
        ...
        #common_genes = [x for x in self._trn_X.columns if x in st_df.columns]
    ...
    def classify(self, data, n_jobs = 1, normalized = True, logarithmized = True, verbose = True, layer = None):
        ...
        trn_X, tst_X, st_df = self._normalize(st_df, normalized = normalized, logarithmized = logarithmized)
